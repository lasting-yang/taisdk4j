...
contract CCC {
    ...
    uint Company_id;
    //初始化合约
    constructor (string memory _companyname, string memory _email, string memory _remark, string memory _enode) public{
        Company_id++;
    ...
    //联盟新成员申请
    function applyMember(string memory _companyname, string memory _email, string memory _remark, string memory _enode, address _account) public {
    ...
    //更改企业信息
    function UpdateCompany(uint _companyid, string memory _email, string memory _remark, string memory _enode, uint _stat) public {
    ...
    //查询投票记录
    function ShowBallot(uint _companyid) public view returns (uint ticketNum){
        ticketNum = C_votes[C_company[_companyid].owner].ticketNum;
    }

    //查看挖矿投票数
    function ShowBallotMine(uint _companyid) public view  returns (uint ticketNum){
        ticketNum = C_votes_mine[C_company[_companyid].owner].ticketNum;
    }

    //查询企业是否为poa成员
    function isMember(uint _companyid) public view returns (bool result){
        result = false;
    ...
    //查询企业是否有挖矿打包权限
    function isMemberMine(uint _companyid) public view  returns (bool result){
        result = false;
    ...
    //查询账号是否是poa账号
    function isMemberOwner(address _account) public view  returns (bool result){
        result = false;
    ...
    //查询企业的所有信息
    function ShowCompany(uint _companyid) public view  returns (string memory R_companyname, string memory R_email, string memory R_remark, address R_owner, string memory R_enode, uint R_stat) {
    ...
    //查询多少企业
    function ShowSum() public view  returns (uint sum) {
        sum = Company_id;
    }
}